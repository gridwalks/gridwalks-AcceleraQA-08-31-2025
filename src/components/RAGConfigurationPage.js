...
// Add this to your RAGConfigurationPage.js component
// Enhanced connection testing functionality

const handleProductionConnectionTest = async () => {
  setIsLoading(true);
  setError(null);
  
  try {
    console.log('🧪 Running production connection test...');
    
    // Call the production test function
    const response = await fetch('/.netlify/functions/connection-test', {
      method: 'GET',
      headers: {
    ...
    reportWindow.document.write(`
      ...
      <html>
      <head>
        <title>AcceleraQA Production Connection Test Report</title>
        <style>
          body { 
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>🔍 AcceleraQA Production Connection Test</h1>
            <div class="status ${testResults.summary?.healthScore >= 80 ? 'success' : 
                                 testResults.summary?.healthScore >= 50 ? 'warning' : 'error'}">
              ${testResults.summary?.overallStatus || 'Unknown Status'}
            </div>
            <p>Health Score: ${testResults.summary?.healthScore || 0}%</p>
          </div>
          
          <div class="test-section">
            <div class="test-title">📋 Test Summary</div>
            <p><strong>Total Tests:</strong> ${testResults.summary?.totalTests || 0}</p>
            <p><strong>Successful:</strong> ${testResults.summary?.successfulTests || 0}</p>
            <p><strong>Failed:</strong> ${testResults.summary?.failedTests || 0}</p>
            <p><strong>Production Ready:</strong> ${testResults.summary?.readyForProduction ? 'Yes ✅' : 'No ❌'}</p>
          </div>
          
          ${Object.entries(testResults.tests || {}).map(([testName, testResult]) => `
            <div class="test-section ${testResult.success ? 'test-success' : 'test-error'}">
              <div class="test-title">
                ${testResult.success ? '✅' : '❌'} ${testName.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}
              </div>
              <div class="code">${JSON.stringify(testResult, null, 2)}</div>
            </div>
          `).join('')}
          ...
          ${testResults.recommendations && testResults.recommendations.length > 0 ? `
            <div class="recommendations">
              ...
              ${testResults.recommendations.map(rec => `<p>${rec}</p>`).join('')}
            </div>
          ` : ''}
          
          <div class="timestamp">
            Test completed at: ${testResults.timestamp}<br>
    ...
    setDebugInfo({
      success: testResults.summary?.healthScore >= 50,
      productionTest: testResults,
      timestamp: new Date().toISOString()
    });
    
    if (testResults.summary?.healthScore < 50) {
      setError(`Production test failed (${testResults.summary?.healthScore}% health). Check the detailed report.`);
    }
    
  } catch (error) {
    console.error('Production connection test failed:', error);
    setError(`Production test failed: ${error.message}`);
    setDebugInfo({
      success: false,
      error: error.message,
      timestamp: new Date().toISOString()
...
// Add this button to your RAG Configuration Debug tab:
/*
<button
  onClick={handleProductionConnectionTest}
  disabled={isLoading}
  className="px-6 py-3 bg-purple-600 text-white font-medium rounded-md hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center space-x-2"
>
  {isLoading ? (
    <Loader className="h-4 w-4 animate-spin" />
  ) : (
  ...
  <span>Run Production Connection Test</span>
</button>
*/
