...
class AuthService {
  constructor() {
    ...
    this.cachedUser = null; // Add user caching
  }
  ...
  async getUser() {
    ...
    try {
      ...
      this.cachedUser = enrichedUser; // Cache the user
      return enrichedUser;
    } catch (error) {
  ...
  async getToken() {
    ...
    try {
      ...
      // FIXED: Handle both JWT and JWE tokens
      try {
        ...
        if (tokenParts.length === 3) {
          ...
          console.log('JWT Token subject:', decoded.sub);
          console.log('JWT Token audience:', decoded.aud);
        } else if (tokenParts.length === 5) {
          // JWE (encrypted JWT) - can't decode client-side
          console.log('JWE token detected - cannot decode client-side');
          // Set a reasonable expiry (1 hour from now)
          this.tokenExpiry = Date.now() + (60 * 60 * 1000);
        } else {
          console.warn('Unknown token format with', tokenParts.length, 'parts');
          // Set a conservative expiry
